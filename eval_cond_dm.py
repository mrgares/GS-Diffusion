...
def evaluate_model(
    ...
    model = load_conditional_diffusion_unet(freeze=False).to(device)
    ...
    # Project 3-channel conditioning images to expected embedding dim
    conditioning_proj = torch.nn.Linear(3, model.config.cross_attention_dim).to(device)
    ...
    with torch.no_grad():
        for i, (inputs, targets, token_ids) in enumerate(tqdm(dataloader, desc="Evaluating")):
            ...
            conditioning = conditioning_proj(conditioning)

            # Start from pure noise
            noisy = torch.randn_like(targets)
            ...
            for t in scheduler.timesteps:
                ...
                noisy = scheduler.step(model_output, t, noisy).prev_sample

            outputs = noisy  # final prediction
